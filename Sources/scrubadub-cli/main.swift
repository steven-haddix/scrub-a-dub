...
guard let input = String(data: raw, encoding: .utf8) else {
    FileHandle.standardError.write(Data("scrubadub-cli: input is not valid UTF-8\n".utf8))
    exit(1)
}
...
let stats = "scrubadub-cli: stripped \(result.strippedCharCount) chars from \(result.lineCount) lines\n"
